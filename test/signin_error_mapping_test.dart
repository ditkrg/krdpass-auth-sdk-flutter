...
void main() {
  ...
  Future<void> emitSignInError(String code) async {
    ...
    await messenger.handlePlatformMessage(
      channel.name,
      codec.encodeMethodCall(MethodCall('onSignInResult', <String, dynamic>{
        'error': code,
  ...
  final cases = <String, Matcher>{
    ...
    'authentication_failed': isA<KrdpassAuthenticationException>(),
  };
  ...
  for (final entry in cases.entries) {
    ...
    test('signIn maps native "$code" to the typed exception', () async {
      ...
      await Future<void>.delayed(Duration.zero); // let signIn invoke the channel
      await emitSignInError(code);
      await expectation;
    });
  }

  test('an unknown native code preserves the code on KrdpassAuthenticationException',
      () async {
    await (client = KrdpassAuth.instance).initialize(config: config);
    ...
    final expectation = expectLater(
      future,
      throwsA(isA<KrdpassAuthenticationException>()
          .having((e) => e.code, 'code', 'provider_not_installed')),
    );
